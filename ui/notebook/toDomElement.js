  ...
  for (const note of notebook) {
    ...
    if (note.md) {
      ...
      if (typeof note.md !== 'string') {
        throw Error(
          `Expected note.md to be string but received: ${
            note.md
          } json=${JSON.stringify(note.md)}`
        );
      }
...
export const getNotebookControlData = async (notebookPath) => {
  const data = {};
  ...
  for (let nthNotebook = 0; nthNotebook < notebooks.length; nthNotebook++) {
    ...
    const inputs = notebook.getElementsByClassName('note control enabled');
