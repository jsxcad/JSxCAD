...
var jsdocExtractor_1 = jsdocExtractor;
...
const toUserGuide = async ({ api, paths, root }) => {
  const markdowns = [];
  for (const path of paths) {
    for (const [buffer] of jsdocExtractor_1(await readFile(path))) {
  ...
  for (const { patch, options, text } of patches) {
    ...
    markdownHtml = markdownHtml.replace(patch, `<img src="data:image/png;base64,${fromByteArray_1(png)}">`);
  }
  const html = `
<html>
 <head>
