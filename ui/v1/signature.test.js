...
test('Circle.ofApothem', t => {
  const signature = toSignature('Circle.ofApothem(apothem:number = 1, { sides:number = 32 }) -> Shape');
  t.deepEqual(signature,
              {
                ...
                outputType: 'Shape',
                string: 'Circle.ofApothem(apothem:number = 1, { sides:number = 32 }) -> Shape'
              });
});
...
test('output', t => {
  const signature = toSignature('Shape -> .writeStl(path:string, { size:number })');
  t.deepEqual(signature,
              {
                inputType: 'Shape',
  ...
  t.deepEqual(snippet,
              {
                ...
                type: 'snippet',
                docHTML: 'Circle.ofApothem(apothem:number = 1, { sides:number = 32 }) -> Shape'
              });
});
