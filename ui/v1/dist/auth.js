...
define("./auth.js",[],function () { 'use strict';
            ...
            const checkIsWebWorker = () => {
              try {
                return self.constructor && self.constructor.name === 'DedicatedWorkerGlobalScope';
              } catch (e) {
                return false;
              }
            };
            ...
            const db = localForageDb; // export const db = idbKeyvalDb;
            ...
            const tellServices = statement => {
              for (const {
                tell
              } of [...idleServices, ...activeServices]) {
                tell(statement);
              }
            };
            /* global self */
            ...
            const {
              serialize
            } = v8$1; // FIX Convert data by representation.
            ...
            const writeFile = async (options, path, data) => {
              ...
              if (workspace !== originalWorkspace) {
                info(`Write ${path} of ${workspace}`); // Switch to the source filesystem, if necessary.
              ...
              if (!ephemeral && base !== undefined) {
                ...
                if (isNode) {
                  try {
                    await promises$3.mkdir(dirname(persistentPath), {
                      recursive: true
                    });
                  } catch (error) {
                    throw error;
