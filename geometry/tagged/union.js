...
const unionImpl = (baseGeometry, ...geometries) => {
  ...
  for (const { solid, tags } of getSolids(baseGeometry)) {
    result.disjointAssembly.push({ solid: solidUnion(solid, ...solids), tags });
  }
  ...
  for (const { z0Surface, tags } of getZ0Surfaces(baseGeometry)) {
    if (surfaces.length === 0) {
      result.disjointAssembly.push({ z0Surface: z0SurfaceUnion(z0Surface, ...z0Surfaces), tags });
    } else {
      result.disjointAssembly.push({ surface: surfaceUnion(z0Surface, ...z0Surfaces, ...surfaces), tags });
    }
  }
  for (const { surface, tags } of getSurfaces(baseGeometry)) {
    result.disjointAssembly.push({ surface: surfaceUnion(surface, ...surfaces, ...z0Surfaces), tags });
  }
  // Paths.
