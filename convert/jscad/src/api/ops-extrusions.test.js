...
test("linear_extrude (height, twist, slices, center)", (t) => {
  const op1 = square();
  const obs = linear_extrude(
    { height: 10, twist: 360, slices: 50, center: true },
    op1
  );
  ...
  const expLastPoly = {
    vertices: [
      { pos: { _x: 3.3306690738754696e-16, _y: 1, _z: 5 } },
      {
        pos: {
          _x: 1.1174479348787818,
          _y: 0.8667814677501742,
          _z: 4.800000000000001,
        },
      },
      {
        pos: {
          _x: 0.12533323356430387,
          _y: 0.9921147013144779,
          _z: 4.800000000000001,
        },
      },
    ],
    ...
    plane: {
      normal: {
        _x: 0.12523593496267418,
        _y: 0.9913445035756271,
        _z: 0.03939588588188166,
      },
      w: 1.188323932985035,
    },
  };
...
test("rotate_extrude (custom negative angle, custom start angle)", (t) => {
  const op1 = square();
  const obs = rotate_extrude(
    { angle: -20, startAngle: 27 },
    op1.translate([4, 0, 0])
  );
...
test("rectangular_extrude ", (t) => {
  ...
  const expLastPoly = {
    ...
    shared: { color: null, tag: 1612 },
    plane: {
      normal: { _x: 0.7071067811865476, _y: -0.7071067811865476, _z: 0 },
      w: -13.642135623730951,
    },
  };
