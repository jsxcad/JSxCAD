...
function comparePositonVertices(obs, exp) {
  for (let index = 0; index < obs.length; index++) {
    let side = obs[index];
    const same =
      side.vertex0.pos._x === exp[index][0][0] &&
      side.vertex0.pos._y === exp[index][0][1] &&
      side.vertex1.pos._x === exp[index][1][0] &&
      side.vertex1.pos._y === exp[index][1][1];
...
test('circle (defaults)', (t) => {
  ...
  const expected = [
    [1.9807852804032304, 0.8049096779838713, 2, 1],
...
test('circle (custom radius)', (t) => {
  ...
  sideEquals(t, obs.sides[0], [
    [19.8078528040323, 8.049096779838713],
    [20, 10],
  ]);
  sideEquals(t, obs.sides[obs.sides.length - 1], [
    [19.238795325112864, 6.173165676349096],
    [19.8078528040323, 8.049096779838713],
  ]);
});
...
test('circle (custom radius, custom resolution, object as parameter)', (t) => {
  ...
  sideEquals(t, obs.sides[0], [
    [13.090169943749473, 0.4894348370484636],
    [20, 10],
  ]);
  sideEquals(t, obs.sides[obs.sides.length - 1], [
    [1.9098300562505255, 4.12214747707527],
    [13.090169943749473, 0.4894348370484636],
  ]);
});
