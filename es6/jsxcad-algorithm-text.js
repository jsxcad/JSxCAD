...
Tokenizer.prototype.composeRUD = function (RUDs) {
  ...
  const hasFAILObject = (obj) =>
    typeof obj === "object" && obj.hasOwnProperty("FAIL");
...
Bidi.prototype.subscribeArabicForms = function (feature) {
  this.tokenizer.events.contextEnd.subscribe((contextName, range) => {
    if (contextName === "arabicWord") {
      return arabicPresentationForms.call(this.tokenizer, range, feature);
    }
  });
};
...
function Substitution$1(lookups, lookupTable, subtable) {
  ...
  if (subtable.hasOwnProperty("coverage")) {
    this.coverage = new Coverage$1(subtable.coverage);
  }
...
function LookupTable(index, lookups) {
  ...
  this.subtables = lookups[index].subtables.map(
    (subtable) => new Substitution$1(lookups, lookups[index], subtable)
  );
}
...
FeatureQuery.prototype.mapTagsToFeatures = function (features, scriptTag) {
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    const featureLookups = new FeatureLookups(
      lookups,
