...
const random = Shape.registerMethod3(
  'random',
  ['inputGeometry', 'function', 'number', 'number'],
  (geometry) => geometry,
  async (geometry, [_, op = () => (s) => s, count = 1, seed = 0]) => {
    const input = Shape.fromGeometry(geometry);
    const values = [];
