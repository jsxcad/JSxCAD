        ...
        switch (upperCaseCommand) {
          ...
          case "S":
            ...
            if (prevPoint.curve && prevPoint.curve.type === "cubic") {
              ...
              sx1 =
                prevPoint.x < prevPoint.curve.x2
                  ? prevPoint.x - diff.x
                  : prevPoint.x + diff.x;
              sy1 =
                prevPoint.y < prevPoint.curve.y2
                  ? prevPoint.y - diff.y
                  : prevPoint.y + diff.y;
            } else {
          ...
          case "T":
            ...
            if (prevPoint.curve && prevPoint.curve.type === "quadratic") {
              ...
              tx1 =
                prevPoint.x < prevPoint.curve.x1
                  ? prevPoint.x - _diff.x
                  : prevPoint.x + _diff.x;
              ty1 =
                prevPoint.y < prevPoint.curve.y1
                  ? prevPoint.y - _diff.y
                  : prevPoint.y + _diff.y;
            } else {
              tx1 = prevPoint.x;
...
var getPointsFromRectWithCornerRadius = function getPointsFromRectWithCornerRadius(
  _ref11
) {
        ...
        switch (curve.type) {
          case "arc":
            var _point$curve2 = point.curve,
              ...
              largeArcFlag =
                _point$curve2$largeAr === undefined ? 0 : _point$curve2$largeAr,
              rx = _point$curve2.rx,
              ry = _point$curve2.ry,
              _point$curve2$sweepFl = _point$curve2.sweepFlag,
              sweepFlag =
                _point$curve2$sweepFl === undefined ? 0 : _point$curve2$sweepFl,
              _point$curve2$xAxisRo = _point$curve2.xAxisRotation,
              xAxisRotation =
                _point$curve2$xAxisRo === undefined ? 0 : _point$curve2$xAxisRo;
...
const fromSvg = async (input, options = {}) => {
  ...
  const svg = new domParser_3().parseFromString(
    await svgString,
    "image/svg+xml"
  );
