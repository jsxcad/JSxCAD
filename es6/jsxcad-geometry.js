...
const measureBoundingBox = (geometry) =>
  computeBoundingBox(linearize(geometry, filter$K));
...
const Link = (geometries, { close = false, reverse = false } = {}) => {
  ...
  for (const geometry of geometries) {
    linearize(geometry, filter$J, inputs);
  }
...
const computeNormal = (geometry) =>
  Group(computeNormal$1(linearize(geometry, filter$I)));
...
const outline = (geometry, selections = []) => {
  ...
  const count = inputs.length;
  for (const selection of selections) {
    linearize(toConcreteGeometry(selection), filter$B, inputs);
  }
...
const filter$y = ({ type }) => type === 'segments';
...
const filter$t = (noVoid, onlyGraph) => (geometry) =>
  ['graph', 'polygonsWithHoles', 'segments', 'points'].includes(
    geometry.type
  ) &&
...
const clip = (
  ...
  const concreteGeometry = toConcreteGeometry(geometry);
  const inputs = [];
  linearize(concreteGeometry, filter$t(noVoid, onlyGraph), inputs);
  const count = inputs.length;
  for (const geometry of geometries) {
    linearize(geometry, filter$t(noVoid), inputs);
  }
...
const commonVolume = (geometry, modes) => {
  const inputs = linearize(
    geometry,
    filter$t(modes.noVoid, { ...modes, onlyGraph: true })
  );
...
const computeCentroid = (geometry, top, bottom) =>
  Group(computeCentroid$1(linearize(geometry, filter$s)));
...
const computeToolpath = (
  ...
  const inputs = [];
  linearize(geometry, filter$q, inputs);
  const materialStart = inputs.length;
  linearize(material, filter$q, inputs);
...
const eachFaceEdges = (
  ...
  const inputs = linearize(geometry, filter$n);
  const count = inputs.length;
...
const fair = (geometry, selections, { density } = {}) => {
  const inputs = [];
  linearize(geometry, filter$m, inputs);
  const count = inputs.length;
  for (const selection of selections) {
    linearize(selection, filter$m, inputs);
  }
  const outputs = fair$1(inputs, count, density);
  const ghosts = [];
  for (let nth = count; nth < inputs.length; nth++) {
    ghosts.push(hasMaterial(hasTypeGhost(inputs[nth]), 'ghost'));
  }
...
const filterAdds = (noVoid) => (geometry) =>
  filter$g() && isNotTypeGhost(geometry);
