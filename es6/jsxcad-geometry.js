...
const measureBoundingBox = (geometry) =>
  computeBoundingBox(linearize(geometry, filter$J));
...
const computeNormal = (geometry) =>
  Group(computeNormal$1(linearize(geometry, filter$H)));
...
const extrude = (geometry, top, bottom, { noVoid } = {}) => {
  const inputs = linearize(geometry, filter$F(noVoid));
  const count = inputs.length;
...
const filter$x = ({ type }) => type === 'segments';
...
const filter$w = (geometry) => ['graph'].includes(geometry.type);
...
const approximate = (
  ...
  const inputs = linearize(geometry, filter$w);
...
const filter$n = (geometry, parent) =>
  ['graph'].includes(geometry.type) && isNotTypeGhost(geometry);
...
const eachFaceEdges = (
  ...
  const inputs = linearize(geometry, filter$m);
  const count = inputs.length;
