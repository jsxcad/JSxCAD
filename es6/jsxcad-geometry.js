...
const measureBoundingBox = (geometry) =>
  computeBoundingBox(linearize(geometry, filter$M));
...
const computeNormal = (geometry) =>
  Group(computeNormal$1(linearize(geometry, filter$K)));
...
const extrude = (geometry, top, bottom, { noVoid } = {}) => {
  const inputs = linearize(geometry, filter$I(noVoid));
  const count = inputs.length;
