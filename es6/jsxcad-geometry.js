...
const filter$A = ({ type }) => type === 'segments';
...
const serialize = (geometry) => {
  const inputs = [];
  linearize(geometry, filter$w, inputs, /* includeSketches= */ true);
  if (inputs.length === 0) {
    return geometry;
  }
  serialize$1(inputs);
...
const computeCentroid = (geometry, top, bottom) =>
  Group(computeCentroid$1(linearize(geometry, filter$u)));
...
const grow = (geometry, tool) => {
  ...
  const count = inputs.length;
  linearize(tool, filter$i, inputs);
...
const Wrap = (geometries, offset = 1, alpha = 0.1, faceCount = 0, minErrorDrop = 0.01) => {
  const inputs = [];
  for (const geometry of geometries) {
...
const wrap = (geometry, geometries, offset, alpha, faceCount, minErrorDrop) =>
  tag(Wrap([geometry, ...geometries], offset, alpha, faceCount, minErrorDrop), tags(geometry));
