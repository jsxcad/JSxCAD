...
const Stud = (
  diameter = 5,
  height = 1.8,
  { play = 0.1, top = 0.5, expansion = 0.2 } = {}
) =>
  Cylinder.ofDiameter(diameter - play - expansion, height)
    .moveZ(height / 2)
    .with(Cylinder.ofDiameter(diameter - play, top).moveZ(height - top / 2));
...
const StudSheet = (
  ...
  return Cube(width - play * 2, length - play * 2, height).with(
    ...studs,
...
const Socket = (
  diameter = 5.1,
  height = 1.8,
  { gripRingHeight = 0.4, gripRingContraction = 0.1, play = 0.0 } = {}
) => {
  ...
  return assemble(
    // flaired top
    Cylinder.ofDiameter(diameter + play, topHeight).moveZ(
      topHeight / 2 + bottom + gripRingHeight
    ),
    // grip ring
    Cylinder.ofDiameter(
      diameter + play - gripRingContraction,
      gripRingHeight
    ).moveZ(gripRingHeight / 2 + bottom),
    // flaired base
    Cylinder.ofDiameter(diameter + play, bottom).moveZ(bottom / 2)
  );
};
...
const SocketSheet = (
  ...
  length = 32,
  height = 1.8,
  { play = 0.1, studMarginX = 0, studMarginY = 0 } = {}
) => {
  const sockets = [];
...
const readLDraw = async (part) =>
  Shape$1.fromGeometry(await fromLDraw(part));
