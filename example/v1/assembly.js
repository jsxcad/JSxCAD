...
export const main = () => {
  ...
  writeStl({ path: 'tmp/assembly-cube.stl' }, assembly.toSolid({ requires: ['cube'] }));

  // This should produce the cube with a hole in it filled by the cylinder.
  writeStl({ path: 'tmp/assembly-cube-cylinder.stl' }, assembly.toSolid({ requires: ['cube', 'cylinder'] }));

  // This should produce just the cylinder and no cube.
  writeStl({ path: 'tmp/assembly-cylinder.stl' }, assembly.toSolid({ requires: ['cylinder'] }));
  ...
  writeThreejsPage({ cameraPosition: [0, 0, 120], path: 'tmp/assembly.html' },
                   assembly.toSolid({ requires: ['cube'] }),
