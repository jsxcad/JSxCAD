...
export const Head = dispatch(
  ...
  ({ style, radius, thickness }) => {
    ...
    return () => Cylinder({ radius, height: thickness, sides: 8 });
  });
...
export const Shank = dispatch(
  ...
  ({ radius, length }) => {
    assertNumber(radius);
    assertNumber(length);
    return () => Cylinder({ radius, height: length, sides: 4 });
  });
...
const ThreadedRod = ({ radius = 1, height = 1, pitch = 1, sides = 16, play = 0 } = {}) => {
  if (play !== 0) {
    return assemble(ThreadedRod({ radius, height, pitch, sides }).drop(),
                    ThreadedRod({ radius: radius - play, height, pitch, sides }));
  } else {
