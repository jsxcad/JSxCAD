    ...
    for (const surface of alignedSolid) {
      for (const face of surface) {
        ...
        for (const [lastPoint, nextPoint] of getEdges(face)) {
          const edgeId = `face/edge:${ensurePointId(nextPoint)}:${ensurePointId(
            lastPoint
          )}`;
          tags.push(edgeId);
        ...
        faces.push(
          Shape.fromGeometry(
            rewriteTags(tags, [], Polygon.ofPoints(face).op(op).toGeometry())
          ).with(...connectors)
        );
      }
    }
...
const faceEdgesMethod = function () {
  return faceEdges(this);
};
