...
export const toShapes = Shape.registerMethod(
  'toShapes',
  (value) => async (shape) => {
    if (value instanceof Promise) { throw Error('toShapes/promise/1'); }
    if (Shape.isFunction(value)) {
      value = await value(Shape.chain(shape));
    }
    if (value instanceof Promise) { throw Error('toShapes/promise/2'); }
    if (Shape.isShape(value) && value.toGeometry().type === 'group') {
      ...
      for (const geometry of (await value.toGeometry()).content) {
        ...
        out.push(item);
        if (item instanceof Promise) { throw Error('toShapes/promise/2a'); }
      }
      value = out;
    }
    if (value instanceof Promise) { throw Error('toShapes/promise/3'); }
    if (Shape.isArray(value)) {
      ...
      for (const item of value) {
        ...
        if (item instanceof Promise) { throw Error('toShapes/promise/4'); }
        out.push(...await toShapes(item)(shape));
      }
      return out;
    } else {
      if (value instanceof Promise) { throw Error('toShapes/promise/5'); }
