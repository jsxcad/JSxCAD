...
var Clipper = /** @class */ (function () {
  ...
  Clipper.prototype.addPath = function (path, polyType, closed) {
    ...
    try {
      return this._clipper.addPath(
        nativePath,
        nativeEnumConversion_1.polyTypeToNative(this._nativeLib, polyType),
        closed
      );
    } finally {
      nativePath.delete();
  ...
  Clipper.prototype.addPaths = function (paths, polyType, closed) {
    var nativePaths = PathsToNativePaths_1.pathsToNativePaths(
      this._nativeLib,
      paths
    );
    try {
   ...
   */
  Clipper.prototype.executeToPaths = function (
    clipType,
    subjFillType,
    clipFillType,
    cleanDistance
    ...
    try {
      var success = this._clipper.executePathsWithFillTypes(
        nativeEnumConversion_1.clipTypeToNative(this._nativeLib, clipType),
        outNativePaths,
        nativeEnumConversion_1.polyFillTypeToNative(
          this._nativeLib,
          subjFillType
        ),
        nativeEnumConversion_1.polyFillTypeToNative(
      ...
      if (!success) {
        ...
        return PathsToNativePaths_1.nativePathsToPaths(
          this._nativeLib,
          outNativePaths,
          true
        ); // frees outNativePaths
      }
    } finally {
   ...
   */
  Clipper.prototype.executeToPolyTee = function (
    clipType,
    subjFillType,
    clipFillType
  ) {
    ...
    try {
      var success = this._clipper.executePolyTreeWithFillTypes(
        nativeEnumConversion_1.clipTypeToNative(this._nativeLib, clipType),
        outNativePolyTree,
        nativeEnumConversion_1.polyFillTypeToNative(
          this._nativeLib,
      ...
      if (!success) {
        ...
        return PolyTree_1.PolyTree.fromNativePolyTree(
          this._nativeLib,
          outNativePolyTree,
          true
        ); // frees outNativePolyTree
      }
    } finally {
