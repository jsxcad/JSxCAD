...
function tryDelete() {
  ...
  try {
    for (
      var objs_1 = __values(objs), objs_1_1 = objs_1.next();
      !objs_1_1.done;
...
function minkowskiSumPath(nativeLib, pattern, path, pathIsClosed) {
  var patternNativePath = PathToNativePath_1.pathToNativePath(
    nativeLib,
    pattern
  );
  var nativePath = PathToNativePath_1.pathToNativePath(nativeLib, path);
...
function simplifyPolygon(nativeLib, path, fillType) {
  ...
  try {
    nativeLib.simplifyPolygon(
      nativePath,
      outNativePaths,
      nativeEnumConversion_1.polyFillTypeToNative(nativeLib, fillType)
    );
    tryDelete(nativePath);
    return PathsToNativePaths_1.nativePathsToPaths(
      nativeLib,
...
function simplifyPolygons(nativeLib, paths, fillType) {
  ...
  var nativePaths = PathsToNativePaths_1.pathsToNativePaths(nativeLib, paths);
  try {
    nativeLib.simplifyPolygonsOverwrite(
      nativePaths,
      nativeEnumConversion_1.polyFillTypeToNative(nativeLib, fillType)
    );
    return PathsToNativePaths_1.nativePathsToPaths(
