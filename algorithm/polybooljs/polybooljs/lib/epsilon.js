...
function Epsilon(eps) {
	...
	var my = {
		...
		linesIntersect: function (a0, a1, b0, b1) {
			...
			if (Math.abs(axb) <= eps * (n1 + n2)) return false; // lines are coincident
			...
			if (my.pointsSame(pt, a0)) ret.alongA = -1;
			else if (my.pointsSame(pt, a1)) ret.alongA = 1;
			else if (A < 0) ret.alongA = -2;
			else if (A > 1) ret.alongA = 2;

			if (my.pointsSame(pt, b0)) ret.alongB = -1;
			else if (my.pointsSame(pt, b1)) ret.alongB = 1;
			else if (B < 0) ret.alongB = -2;
			else if (B > 1) ret.alongB = 2;
		...
		pointInsideRegion: function (pt, region) {
			...
			for (var i = 0; i < region.length; i++) {
				...
				if (
					...
					((last_x - curr_x) * (y - curr_y)) / (last_y - curr_y) +
						curr_x -
						x >
						eps
				)
