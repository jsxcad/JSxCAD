			...
			function (require, module, exports) {
				...
				PolyBool = {
					...
					selectUnion: function (combined) {
						return {
							segments: SegmentSelector.union(
								combined.combined,
								buildLog
							),
					...
					selectIntersect: function (combined) {
						return {
							segments: SegmentSelector.intersect(
								combined.combined,
								buildLog
							),
							inverted: combined.inverted1 && combined.inverted2,
					...
					selectDifference: function (combined) {
						return {
							segments: SegmentSelector.difference(
								combined.combined,
								buildLog
							),
							inverted: combined.inverted1 && !combined.inverted2,
				...
				function Epsilon(eps) {
					...
					var my = {
						...
						pointsSame: function (p1, p2) {
							return (
								my.pointsSameX(p1, p2) && my.pointsSameY(p1, p2)
							);
						},
						pointsCompare: function (p1, p2) {
							// returns -1 if p1 is smaller, 1 if p2 is smaller, 0 if equal
							if (my.pointsSameX(p1, p2))
								return my.pointsSameY(p1, p2)
									? 0
									: p1[1] < p2[1]
									? -1
									: 1;
					...
					toPolygon: function (PolyBool, geojson) {
						...
						function GeoPoly(coords) {
							...
							for (var i = 1; i < coords.length; i++)
								out = PolyBool.selectDifference(
									PolyBool.combine(out, LineString(coords[i]))
								);
						...
						if (geojson.type === "Polygon") {
							...
							for (var i = 0; i < geojson.coordinates.length; i++)
								out = PolyBool.selectUnion(
									PolyBool.combine(
										out,
										GeoPoly(geojson.coordinates[i])
									)
								);
					...
					function calculate(
						...
						function statusCompare(ev1, ev2) {
							...
							if (eps.pointsCollinear(a1, b1, b2)) {
								...
								return eps.pointAboveOrOnLine(a2, b1, b2)
									? 1
									: -1;
							}
						...
						function checkIntersection(ev1, ev2) {
							...
							if (i === false) {
								...
								if (
									eps.pointsSame(a1, b2) ||
									eps.pointsSame(a2, b1)
								)
								...
								var a1_between =
									!a1_equ_b1 && eps.pointBetween(a1, b1, b2);
								var a2_between =
									!a2_equ_b2 && eps.pointBetween(a2, b1, b2);
						...
						while (!event_root.isEmpty()) {
							...
							if (ev.isStart) {
								...
								var above = surrounding.before
									? surrounding.before.ev
									: null;
								var below = surrounding.after
									? surrounding.after.ev
									: null;
								...
								if (selfIntersection) {
									...
									if (toggle)
										ev.seg.myFill.above = !ev.seg.myFill
											.below;
									else
										ev.seg.myFill.above =
											ev.seg.myFill.below;
						...
						return {
							calculate: function (
								...
								segments1.forEach(function (seg) {
									eventAddSegment(
										segmentCopy(seg.start, seg.end, seg),
										true
									);
								});
