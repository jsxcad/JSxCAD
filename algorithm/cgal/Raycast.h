...
static int Raycast(Geometry* geometry, double x_start, double x_stride, size_t x_steps, double y_start, double y_stride, size_t y_steps, double z, const std::function<void(size_t x_step, size_t y_step, const IK::FT&, const IK::Vector_3&)>& emit) {
          ...
          for (double y = y_start; y_step < y_steps; y += y_stride, y_step++) {
            ...
            if (intersection) {
              ...
              if (p) {
                ...
                CGAL::Surface_mesh<IK::Point_3>::Face_index facet = intersection->second;
