...
class Clean_cost {
 public:
  ...
  bool is_safe_to_move(const Surface_mesh& m, const Vertex_point_map& p,
                       const Halfedge_index start) const {
  ...
  boost::optional<typename Profile::FT> operator()(
    ...
    // Collapsing an edge to its midpoint moves its same-face adjacent edges.
    ...
    // Collapsing an edge is ok if this edge is collinear, and all other edges
    // are coplanar with their neighbors.

    // This won't clean up the mesh perfectly, since it requires both ends of
    // the edge to be mobile. But I think we should be able to collapse if only
    ...
    if (is_safe_to_move(m, p, profile.v0_v1()) &&
        is_safe_to_move(m, p, profile.v1_v0())) {
      return FT(0);
    } else {
      return boost::none;
    }
...
const Surface_mesh* CleanSurfaceMesh(const Surface_mesh* input) {
  ...
  Clean_cost<Kernel> cost;
  CGAL::Surface_mesh_simplification::Midpoint_placement<Surface_mesh> placement;
...
EMSCRIPTEN_BINDINGS(module) {
  ...
  emscripten::function("ApproximateSurfaceMesh", &ApproximateSurfaceMesh,
                       emscripten::allow_raw_pointers());
  emscripten::function("CleanSurfaceMesh", &CleanSurfaceMesh,
                       emscripten::allow_raw_pointers());
